...
def test_packaging_assumptions_accept_quarantined_dist_output_when_live_dist_is_missing(tmp_path: Path) -> None:
    (tmp_path / "bin").mkdir()
    (tmp_path / "docs").mkdir()
    runtime_dir = tmp_path / "runtime"
    runtime_dir.mkdir()
    quarantined_dist_dir = tmp_path / ".quarantine" / "2026-04-22_quarantine_wave_01" / "dist" / "Guppy"
    quarantined_dist_dir.mkdir(parents=True)
    with (quarantined_dist_dir / "Guppy.exe").open("wb") as handle:
        handle.truncate(51 * 1024 * 1024)
    (tmp_path / "guppy_launcher.py").write_text("print('launcher')\n", encoding="utf-8")
    (tmp_path / "bin" / "Guppy.spec").write_text("a = Analysis([])\n", encoding="utf-8")
    (tmp_path / "bin" / "build_executable.bat").write_text(
    ...
    assert ok is True
    status_by_label = {item.label: item for item in statuses}
    assert "found quarantined package output" in status_by_label["dist artifact layout"].detail
