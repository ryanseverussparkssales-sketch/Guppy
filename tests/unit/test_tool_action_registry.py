...
class TestRegistryCatalogCoverage:
    def test_all_catalog_keys_in_registry(self) -> None:
        ...
        from ui.launcher.views.tools_view_cards import INSTANCE_TOOL_CATALOG

        catalog_keys = {t["key"] for t in INSTANCE_TOOL_CATALOG}
        registry_keys = set(TOOL_ACTION_REGISTRY.keys())
        missing = catalog_keys - registry_keys
        assert not missing, (
            f"INSTANCE_TOOL_CATALOG keys missing from TOOL_ACTION_REGISTRY: {missing}\n"
            "Add an entry to tool_action_registry.py for each new tool."
        )
    ...
    def test_registry_and_catalog_labels_are_consistent(self) -> None:
        ...
        for tool in INSTANCE_TOOL_CATALOG:
            ...
            assert entry.label == tool["name"], (
                f"Label mismatch for {key!r}: registry={entry.label!r}, catalog={tool['name']!r}"
            )
