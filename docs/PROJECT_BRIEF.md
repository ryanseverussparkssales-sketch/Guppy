...
14. **Three-Surface Architecture Complete (2026-04-29):** P7 shipped. The single `AssistantView` has been fully replaced by three dedicated surfaces — Companion (`/companion`), Workspace (`/workspace`), Codespace (`/codespace`) — each with dedicated model affinity, cross-surface SSE events, task escalation, and Docker sandbox. The old 5-hub Qt launcher architecture described above in items 1–12 is historical; the live product is the 3-surface web UI. See `CLAUDE.md` and `docs/MASTER_PHASE_PLAN.md` for the authoritative current architecture.
15. **Phase 6 Hardening Complete (2026-05-02):** JWT auth fix on `/repair-token/refresh`, 6-gap stability hardening (surface-locked routing, watchdog, KV cache warming, strict-local mode), 7 best-practices items (workspace tool execution, streaming TTS sentence-boundary chunking, SSE exponential backoff, OOM error parsing, grammar-constrained tool calls), Docker packaging, semantic fallback unit tests, and tool-call log audit all complete.
...
| P6 | Platform Hardening + Packaging Readiness | **complete** | Reduce architectural hotspot risk while explicitly stabilizing local/cloud chat, model switching, stats fidelity, guarded PC-control flows, and dual web/desktop surface parity | Release remains repeatable; chat/runtime switching is stable; stats stop drifting from runtime truth; PC-control stays powerful but bounded; web and desktop remain parallel clients over shared contracts | Completed May 2, 2026 |
| P7 | Three-Surface Architecture | **complete** | Replace single AssistantView with Companion (voice/vision/personality), Workspace (operations hub), and Codespace (Docker sandbox + self-triage) | Each surface renders, has dedicated model affinity, shares memory substrate, and cross-surface task escalation works end-to-end | Completed April 29, 2026 — see `docs/MASTER_PHASE_PLAN.md` |
| P8 | Voice Pipeline — Wake-Word + TTS Streaming + Mic | active | Wire Porcupine wake-word, true chunked TTS streaming (Kokoro local + ElevenLabs), and validated mic capture backend | Wake-word detection fires in real-time from mic stream; TTS yields sentences as they are synthesized; mic capture works with sounddevice | In progress |
