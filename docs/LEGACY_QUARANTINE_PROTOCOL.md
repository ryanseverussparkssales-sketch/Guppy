...
- `compat_shims/legacy_surfaces/__init__.py` package marker (quarantine boundary)
...
- `compat_shims/legacy_surfaces/`: quarantine boundary only; supported entrypoints must not depend on it.
...
- April 20, 2026 (hygiene pass): removed `guppy_ui_legacy.py`, `merlin_ui_legacy.py`, and `council_ui_legacy.py` after reference scan confirmed no supported entrypoint imports them.
