...
**Prep Window:** April 15 â€” June 14, 2026 (2 months to ramp)  
**Active Development:** June 15 â€” September 30 (3.5 months)

---

## Product Positioning: Primary vs Supporting Surfaces

**Primary Surface (Home Tab):** Active instance chat interface â€” the main product offering where users chat with their selected models and personas
...
1. **Instance Manager Tab** (`ui/launcher/views/instance_manager_view.py`)
   ...
   - Double-click to activate instance â†’ switch home tab to that instance
...
2. **Instance Creation Modal** (`ui/launcher/components/instance_editor_modal.py`)
   ...
   - Save â†’ writes to `config/instances.json`
...
5. **Inter-Agent Communication Bridge** (extend `guppy_api.py`)
   - New endpoint: `POST /instances/{name}/query` â†’ send message to background instance
...
1. **Assistant Home Tab** (enhance existing `ui/launcher/views/assistant_view.py`)
   ...
   - Instance indicator in top-left (clickable â†’ switch instance)
   - **Lines of code est.:** ~200

2. **Quick Model Selection** (in home tab header)
   - Dropdown: "Model: [Merlin â–¼]"
...
1. **Persona Form Widget** (`ui/launcher/components/persona_form.py`)
   - Tone slider (0â€“10): formal â†’ conversational
   - Verbosity slider (0â€“10): terse â†’ verbose
   - Teaching style dropdown: Socratic | Direct | Example-Driven | Coaching
   - Scope radio buttons: Global | Per-Model Override
   - Save button â†’ writes to `runtime/persona_config.json`
...
4. **Form Integration** (launcher_window.py)
   ...
   - Save flow: validate â†’ write JSON â†’ reload system prompt
...
1. **Model Assignment Card** (`ui/launcher/components/model_assignment_card.py`)
   ...
   - Save â†’ writes to `runtime/provider_registry.json`
   - **Lines of code est.:** ~500

2. **Health Status Badges** (extend existing badge renderer)
   - âœ“ Ready (model responding <2s)
   - âš  Slow (model responding 2â€“10s)
   - âœ— Offline (model not responding)
   - Fetch from `/status` API endpoint
   - cache for 30s to avoid hammering
   - **Lines of code est.:** ~150

3. **Route Visualizer** (optional diagram)
   - Flow chart: task â†’ router â†’ model1 â†’ model2 â†’ model3
   - Show fallback chain visually
   - Clickable nodes â†’ show system prompt for that model
...
### Dependencies
- [ ] `/status` API endpoint must return model health (EXISTING â€” check guppy_api.py)
- [ ] Provider/model routes schema defined (EXISTING â€” check runtime/provider_registry.json)
...
2. **Voice Assignment Form**
   ...
   - Per-persona override (Merlin â†’ ElevenLabs, etc.)
   - Per-model override (guppy-fast â†’ system EN, guppy â†’ Kokoro)
   - Preview button â†’ speaker icon, play 3-second sample
   - **Lines of code est.:** ~300
...
4. **Voice Playback Integration** (extend `src/guppy/voice/voice.py`)
...
### Dependencies
- [ ] `src/guppy/voice/voice.py` must support voice selection per-call
...
1. **Tool Card Template** (`ui/launcher/components/agent_tool_card.py`)
   ...
   - Status badge: âœ“ Ready | ðŸ”§ Wired | â± Coming M3 | ðŸ”’ Instance-Restricted
...
1. **Recovery Action Cards** (`ui/launcher/components/recovery_action_card.py`)
   - Card 1: "Warmup â€” Refresh model cache, reload voice engine"
   - Card 2: "Restart Daemon â€” Hard reset background processes"
   - Card 3: "Audit Runtime â€” Check logs, schema validation, health"
...
### Dependencies
- [ ] `/repair` endpoint working (existing â€” check guppy_api.py)
...
2. **Dry-Run Review Workflow**
   - Task runs â†’ output staged to `runtime/offhours_results/dry_run/`
...
### Dependencies
- [ ] `apply_patch` tool working (EXISTING â€” done in off-hours extension)
...
### **May 1â€“31 (Core Implementation: Foundation + Configuration)**
...
### **June 15 â€“ Sep 30 (Refinement, Scale, and Advanced Features)**
