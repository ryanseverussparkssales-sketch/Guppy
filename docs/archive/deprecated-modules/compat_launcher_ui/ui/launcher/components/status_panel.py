...
class StatusPanel(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        hero = QFrame()
        hero.setStyleSheet(self._card_style(T.SURFACE_ELEVATED_92))
        ...
        title.setStyleSheet(
            f"color: {T.ACCENT_TEAL_TEXT}; font-family: '{T.FONT_SERIF}';"
            f"font-size: {T.FS_TITLE + 1}pt; font-style: italic; font-weight: 700;"
        )
        ...
        self._extras_btn.setStyleSheet(
            "QPushButton {"
            f" background: {T.SURFACE_BASE_90}; color: {T.DIM}; border: 1px solid {T.BORDER_SOFT_60};"
            f" border-radius: 14px; padding: 4px 10px; font-family: '{T.FF_MONO}'; font-size: {T.FS_TINY}pt; font-weight: 700; }}"
            f"QPushButton:hover {{ border-color: {T.ACCENT_TEAL}; color: {T.ACCENT_TEAL_TEXT}; background: {T.WHITE}; }}"
        )
        ...
        self._workspace_lbl.setStyleSheet(
            ...
            f" border-radius: 10px; padding: 4px 8px; font-family: '{T.FF_MONO}';"
        ...
        self._activity_lbl.setStyleSheet(
            f"color: {T.TEXT_DIM_72}; font-family: '{T.FF_BODY}';"
            f" font-size: {T.FS_SMALL}pt; font-weight: 600;"
        )

        hero_layout.addWidget(self._workspace_lbl)
        hero_layout.addWidget(self._tray_status_lbl)
        hero_layout.addWidget(self._activity_lbl)
        outer.addWidget(hero)

        tools_frame = QFrame()
        tools_frame.setObjectName("drawer_actions")
        tools_frame.setStyleSheet(self._card_style(T.SURFACE_ELEVATED_92))
        ...
        self._tool_hint_lbl.setStyleSheet(
        ...
        spaces_frame.setStyleSheet(self._card_style(T.SURFACE_ELEVATED_92))
        ...
        self._spaces_summary_lbl.setStyleSheet(
            f"color: {T.TEXT_DIM_72}; font-family: '{T.FF_BODY}'; font-size: {T.FS_SMALL}pt; font-weight: 600;"
        )
        ...
        media_frame.setStyleSheet(self._card_style(T.SURFACE_ELEVATED_92))
        ...
        artwork = QFrame()
        artwork.setFixedHeight(126)
        artwork.setStyleSheet(
            f"background: qlineargradient(x1:0,y1:0,x2:1,y2:1, stop:0 {T.ART_RED}, stop:0.35 {T.ART_GOLD}, stop:0.72 {T.ART_PINK}, stop:1 {T.ART_BLUE});"
        ...
        self._media_title_lbl.setStyleSheet(
            f"color: {T.ACCENT_TEAL_TEXT}; background-color: {T.SURFACE_ELEVATED_92};"
        ...
        self._media_subtitle_lbl.setStyleSheet(
            ...
            f" border-radius: 12px; padding: 4px 8px; font-family: '{T.FF_BODY}';"
            f" font-size: {T.FS_SMALL}pt; font-weight: 600;"
        )
        art_layout.addWidget(self._media_subtitle_lbl)
        art_gesture = QLabel("CALM / FOCUS / CONTEXT")
        art_gesture.setStyleSheet(
            f"color: {T.ACCENT_TEAL_TEXT}; background-color: {T.SURFACE_ELEVATED_88};"
            f" border-radius: 9px; padding: 2px 8px; font-family: '{T.FF_MONO}'; font-size: 9pt; letter-spacing: 2px; font-weight: 700;"
        )
        ...
        for label in ("PREV", "PLAY", "NEXT"):
            ...
            btn.setStyleSheet(
                "QPushButton {"
                f" background: {T.SURFACE_BASE_90};"
                f" color: {T.TEXT_DIM_78};"
                f" border: 1px solid {T.BORDER_MID_30};"
                " border-radius: 14px;"
                f" padding: 7px 10px; font-family: '{T.FF_MONO}'; font-size: {T.FS_TINY}pt; font-weight: 700;"
                "}"
            )
        ...
        outer.addStretch()
        self._set_tray_status("READY", T.STATUS_SUCCESS)
    ...
    def _tool_button_style(enabled: bool) -> str:
        if enabled:
            return (
                "QPushButton {"
                f" background: {T.WHITE}; color: {T.TEXT}; border: 1px solid {T.BORDER_MID_30};"
                f" border-radius: 16px; font-family: '{T.FF_MONO}'; font-size: {T.FS_TINY}pt; font-weight: bold; }}"
                f"QPushButton:hover {{ border-color: {T.ACCENT_TEAL}; color: {T.ACCENT_TEAL_TEXT}; background: {T.SURFACE_ELEVATED_92}; }}"
            )
        return (
            "QPushButton {"
    ...
    def _space_button_style() -> str:
        return (
            "QPushButton {"
            f" background: {T.SURFACE_BASE_90}; color: {T.TEXT}; border: 1px solid {T.BORDER_MID_30};"
            f" border-radius: 14px; padding: 0 10px; font-family: '{T.FF_MONO}'; font-size: {T.FS_TINY}pt; font-weight: 700; }}"
            f"QPushButton:hover {{ border-color: {T.ACCENT_TEAL}; color: {T.ACCENT_TEAL_TEXT}; background: {T.WHITE}; }}"
        )

    def _set_tray_status(self, text: str, color: str) -> None:
        self._tray_status_lbl.setText(text)
        self._tray_status_lbl.setStyleSheet(
            f"color: {color}; background-color: {T.SURFACE_BASE_90};"
            f" border-radius: 10px; padding: 4px 8px; font-family: '{T.FF_MONO}';"
            f" font-size: {T.FS_TINY}pt; letter-spacing: 1px; font-weight: bold;"
        )
    ...
    def update_status(self, data: dict) -> None:
        ...
        if query in {"-", "â€”"}:
            query = ""
        self._set_tray_status(f"READY / {model}", T.STATUS_SUCCESS)
        if query:
    ...
    def update_agent_status(self, agent: str, online: bool, last_seen: str = "-", load_pct: float | None = None) -> None:
        ...
        if online:
            self._set_tray_status("LOCAL READY", T.GREEN)
            return
        self._set_tray_status("LOCAL OFFLINE", T.ERROR)
    ...
    def set_recovery_outcome(self, action: str, ok: bool, summary: str) -> None:
        ...
        self._set_tray_status(f"RECOVERY / {state}", color)
