...
_SEED_TOOLS = [
    ...
    ("api_request",     "API Request",      "Make HTTP requests to external APIs",                                "search",  "builtin", '{"type":"object","properties":{"url":{"type":"string"},"method":{"type":"string","enum":["GET","POST","PUT","PATCH","DELETE","HEAD"],"default":"GET"},"headers":{"type":"object"},"body":{},"timeout":{"type":"number","default":15}},"required":["url"]}', 1),
    # ── File & Shell ─────────────────────────────────────────────────────────
    ("read_file",       "Read File",        "Read contents from a local file",                                    "file",    "builtin", '{"type":"object","properties":{"path":{"type":"string"}},"required":["path"]}', 1),
    ("write_file",      "Write File",       "Write or overwrite a local file (restricted to allowed directories)",        "file",    "builtin", '{"type":"object","properties":{"path":{"type":"string"},"content":{"type":"string"},"mode":{"type":"string","enum":["overwrite","append"],"default":"overwrite"}},"required":["path","content"]}', 1),
