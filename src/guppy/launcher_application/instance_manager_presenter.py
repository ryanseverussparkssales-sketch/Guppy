...
def build_connector_binding_feedback(
    ...
    *,
    ...
    return (
        ...
        connector_history_line(connector_payload.get("history", {}) if isinstance(connector_payload.get("history"), dict) else {}),
    )
...
def build_instance_manager_state(
    ...
    *,
    ...
    recurring_text = (
        f"Recurring context: {workspace_saved_context(active_payload)} | {workspace_reentry_hint(active_type)} | {workspace_recent_context(active_payload)}"
        if active_instance
