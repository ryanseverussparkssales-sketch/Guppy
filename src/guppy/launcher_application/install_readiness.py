...
from src.guppy.launcher_application.packaging_audit import packaging_assumption_checks
...
def _core_import_static() -> bool:
    try:
        text = (_REPO_ROOT / "guppy_launcher.py").read_text(
            encoding="utf-8",
            errors="replace",
        )
...
def _packaging_assumption_ok(label: str) -> bool:
    try:
        for item in packaging_assumption_checks(_REPO_ROOT):
            if item.label == label:
                return bool(item.ok)
    except Exception:
        return False
...
INSTALL_READINESS_CHECKS: list[InstallReadinessCheck] = [
    ...
    InstallReadinessCheck(
        ...
        "runtime/ is writable (created if absent)",
        lambda: _dir_writable("runtime"),
    ),
    InstallReadinessCheck(
        "tmp_dir_writable",
        ".tmp/dev-workflow/reports/ is writable",
    ...
    InstallReadinessCheck(
        "release_handoff_current",
        "runtime/windows_release_receipt.json and summary reflect packaging or release-dry-run evidence",
        lambda: _packaging_assumption_ok("release handoff contract"),
    ),
    InstallReadinessCheck(
        "dist_artifact_layout",
        "dist/Guppy/Guppy.exe or dist/Guppy.exe exists as a real packaged build",
        lambda: _packaging_assumption_ok("dist artifact layout"),
    ),
]
